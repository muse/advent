defmodule Day.KnotHash do
  ...
  length plus the skip size.  Increase the skip size by one.
  ...
  into 2 1 0), we get ([2] 1 0) 3 4.  Then, the current position moves forward
  by the length, 3, plus the skip size, 0: 2 1 0 [3] 4. Finally, the skip size
  increases to 1.
  ...
  position moves forward by the length plus the skip size, a total of 5, causing
  it not to move because it wraps around: 4 3 0 [1] 2. The skip size increases
  to 2.

  The third length, 1, selects a sublist of a single element, and so reversing
  it has no effect.  The current position moves forward by the length (1) plus
  the skip size (2): 4 [3] 0 1 2. The skip size increases to 3.
  ...
  @typep collective :: [integer, ...]

  def reverse() do

  end

  def prepend([value | values], to) do
    prepend(values, [value | to])
  end
  def prepend([], to) do
    to
  end

  # def take(collective, 0, amount) when amount < @size do
  #   {left, right} =
  #     Enum.split(collective, amount)
  #   prepend(left, right)
  # end
  #
  def take(collective, from, amount) do
    {left, right} =
      Enum.split(collective, from)
    {left, right} =
      right
      |> Enum.reverse
      |> prepend(left)
  ...
  def enumerate(list, [distance | distances], step) do
    list = take(list, step, distance)
    step = step + 1
    enumerate(list, distances, step)
  end
  def enumerate(list, [], _step) do
    list
  end

  def one(input) do
    lengths = [3, 4, 1, 5]
    # list = @min..@max |> Enum.to_list
       list = [0, 1, 2, 3, 4]

    enumerate(list, lengths, 0)

    # Enum.reduce(lengths, 0, fn l, a ->
    #
    # end)
